...
const coinone = async (obj: { api_key: string; api_secret: string }) => {
  try {
    ...
    const signature = createHmac('sha512', api_secret).update(encodedPayload).digest('hex');
    const options = {
    ...
    const priceRes = await axios
      .get('https://api.coinone.co.kr/public/v2/ticker_new/KRW?additional_data=false')
    ...
    const filtered = res
      ...
      .map((e: any) => {
        return {
          ...
          value: (Number(e.available) + Number(e.limit)) * priceObj[e.currency.toLowerCase()],
        };
      });
